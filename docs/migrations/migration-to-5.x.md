...
##### Flatpickr cons:
  ...
  - not fully ESM ready (it's only partially ESM, for example it is detected as CJS in Slickgrid-React and requires an exception in `allowedCommonJsDependencies`)
...
### Slickgrid-Universal
If you use any of the Slickgrid-Universal extra dependencies then make sure to upgrade them as well to the new major `5.0.0` version so that they work with Slickgrid-React `5.0.0`
