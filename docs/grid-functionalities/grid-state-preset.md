...
What happens when we use the grid `presets` and a [Filter Default SearchTerms](/ghiscoding/slickgrid-react/wiki/Select-Filter#default-search-terms)? In this case, the `presets` will win over filter `searchTerms`. The cascading order of priorities is the following
