...
const Example27: React.FC = () => {
  ...
  function updateFirstRow() {
    ...
    */
    ...
    reactGridRef.current?.gridService.updateItem({ ...item, duration: `11 days`, percentComplete: 77, start: new Date(), finish: new Date(), effortDriven: false });
  }
  ...
  function dynamicallyToggledFirstParent() {
    ...
    if (childItemFound && parentItemFound) {
      reactGridRef.current?.treeDataService.dynamicallyToggleItemState([{ itemId: parentItemFound.id, isCollapsed: !parentItemFound.__collapsed }]);
    }
  }
  ...
  return !gridOptions ? '' : (
    <div id="demo-container" className="container-fluid">
      <h2>
        Example 27: Tree Data <small>(from a flat dataset with <code>parentId</code> references)</small>
      ...
      <div className="row" style={{ marginBottom: '4px' }}>
        <div className="col-md-12">
          ...
          <button onClick={() => dynamicallyChangeFilter()} className="btn btn-outline-secondary btn-xs btn-icon"
          ...
          <button onClick={() => collapseAllWithoutEvent()} className="btn btn-outline-secondary btn-xs btn-icon mx-1"
          ...
          <button onClick={() => dynamicallyToggledFirstParent()} className="btn btn-outline-secondary btn-xs btn-icon"
            data-test="dynamically-toggle-first-parent-btn">
            <span>Dynamically Toggle First Parent</span>
          </button>
          <button onClick={() => reapplyToggledItems()} data-test="reapply-toggled-items-btn"
